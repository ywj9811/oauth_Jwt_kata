...
import code.kata.global.config.security.jwt.principal.PrincipalDetails;
import code.kata.global.config.security.jwt.principal.UserPrincipalDetails;
import code.kata.global.redis.RedisRepository;
...
public class JwtProvider implements InitializingBean {
    ...
    public TokenInfoResponse createToken(Authentication authentication, Provider provider) {
        ...
        switch (provider) {
            case NONE: principal = (UserPrincipalDetails) authentication.getPrincipal();
                break;
            default: principal = (Oauth2PrincipalDetails) authentication.getPrincipal();
                break;
        }
    ...
    public Authentication getAuthentication(String token) {
        ...
        return new UsernamePasswordAuthenticationToken(new UserPrincipalDetails(user), token, authorities);
    }
